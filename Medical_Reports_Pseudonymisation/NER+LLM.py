...
def response_to_question(question,vectordb):
    ...
    llm = ChatCohere(cohere_api_key="RC72GM5L5FHPz6o2VdaKJ2ItqWa8TIMlPX0UeVDv")
    #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
        ...
        with st.spinner("Processing"):
            if pdf is not None:
                ...
                if date_de_naissance:
                    ...
                    cleaned_text = extracted_text.replace(date_dic[min(date_dic)]," DATE_DE_NAISSANCE")
                ...
                code_postal = response_to_question("Dis moi uniquement le code postal du patient sans faire de phrase. Ecris moi le code postal exactement comme il apparait dans le texte. S'il y en a pas dis moi juste : Aucun",vectordb)
                ...
                pseudonymized_text = response_to_question("Voici le texte synonymiser que j'ai fais : "+cleaned_text+"\nCependant à partir du texte original ainsi qu'avec la table suivante : \n"+name_to_id+" Assure toi que j'ai correctement remplacé les noms par les IDs correspondant se trouvant uniquement dans la table que je viens de te donner, donne plus d'importances à cette table, si un nom n'apparaissant pas dans ta table a été remplacé alors laisse le tel qu'il est dans le texte original. De plus si un ID apparait dans le texte alors qu'il n'est pas dans la table remplace le par le nom de base. J’ai également remplacé la date de naissance du patient par DATE_DE_NAISSANCE si elle était présente dans le texte original assure toi que cela à également bien été fait si ce n'est pas le cas fais le. J’ai également remplacé le nom de la ville où habite le patient par le mot VILLE. Assure toi que cela à également bien été fait et vérifie qu'on ne puisse pas savoir où il habite ou bien où il est née, si ce n'est pas le cas fais le, si le mot VILLE n'apparait pas alors remplace : "+ville+" par VLLE. J'ai également remplacé le code postal de la ville associé au patient qui est composé de 5 chiffres (soit où il habite soit où il est née) par CODE_POSTAL. Remplace le code postal suivant : "+code_postal+ "  par CODE_POSTAL si ce n'est pas déja fait. J’ai également remplacé le numéro de téléphone du patient par NUMERO_DE_TELEPHONE s'il est présent dans le texte original assure toi que cela à également bien été fait si ce n'est pas le cas fais le. Assure toi également que les noms des personnes qui n'ont pas été pseudonymisé ne sont pas coupé en plein milieu. Je veux uniquement que tu me donnes le texte bien pseudonymisé en français en traduisant mot pour mot. Important si DATE_DE_NAISSANCE, VILLE , CODE_POSTAL ou NUMERO_DE_TELEPHONE sont présent dans le texte laisse tel quel.",vectordb)
